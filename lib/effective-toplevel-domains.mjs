...
/**
 * @type {string}
 * @see {@link https://publicsuffix.org/list/public_suffix_list.dat}
 */
public_suffix_list = 
...
should_be_included_in_public_suffix_list =
`
icp0.io
ic0.app
`,

effective_toplevel_domains_list = public_suffix_list + should_be_included_in_public_suffix_list,

included_and_excluded_domains =
effective_toplevel_domains_list
// split into lines
.split(/\r?\n/)
// keep lines that are not comments
.filter(line => line.match(/^[^\/][^\/]/))
// keep non-empty lines without spaces
.filter(line => line.match(/^\S+$/)),

effective_toplevel_domains = {
  included: (
    included_and_excluded_domains
  ...
  excluded: (
    included_and_excluded_domains
    // keep lines that start with '*.', as this prefix seems to designate tlds that are not allowed for outside buyers.
    .filter(line => line.match(/^\*\./))
    // remove the '*.' prefix
...
export default effective_toplevel_domains;
export {effective_toplevel_domains};
